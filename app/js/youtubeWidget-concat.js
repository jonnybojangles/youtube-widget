...
;(function(){
	...
	angular.module('youtubeWidget.directives', []).
		...
		run(function($templateCache){
			/*
			* IE: Conditional IE tag is utalized to support a dynamic height two column with one fixed column layout in both IE and FF.
			* In other words A layout can be written for Gecko and Webkit but breaks in IE. A layout can be written for IE and Webkit but breaks in FF
			* */
				...
				function($scope, $element, videoPlayer, videoSearch, $anchorScroll){
					...
					function parseVideoList(data){
						...
						if (
							...
							console.error('Error: youtube-widget: videoList: failed to get video results.');
						}
					}
					/*
